...
public class ArrayGamePublicTest {
	...
	public void testReset() {
		...
		assertNull("priorGuesses after reset",
				gamerA.priorGuesses());
	}
	...
	public void testPriorGuesses() {
		gamerB.reset();
		assertNull("test prior guesses", gamerB.priorGuesses());
	...
	@Test(timeout = 1000)
	public void testPriorGuesses1K2(){
        gamerA.reset();
        // ground truth number is 2000
        assertEquals("gamerA first guess", 1000, gamerA.getGuess());
        assertTrue("gamerA first update", gamerA.updateGuess(3));

        assertEquals("gamerA second guess", 1001, gamerA.getGuess());
        assertTrue("gamerA second update", gamerA.updateGuess(2));

        assertEquals("gamerA third guess", 1010, gamerA.getGuess());
        assertTrue("gamerA third update", gamerA.updateGuess(2));

        assertEquals("gamerA fourth guess", 1100, gamerA.getGuess());
        assertTrue("gamerA fourth update", gamerA.updateGuess(2));

        assertEquals("gamerA fifth guess", 2000, gamerA.getGuess());
        assertTrue("gamerA fifth update", gamerA.updateGuess(4));
